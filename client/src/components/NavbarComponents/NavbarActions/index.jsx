...
const NavbarActions = () => {
  const { toggleIsActive, toggleIsOpen, setIsOpen, isOpen } =
    ...
    <>
      <div className='nav-actions col-span-2 flex justify-end gap-4'>
        ...
        {!token ? (
          <span
            onClick={openProfile}
            className='profile cursor-pointer w-[17px] md:w-[24px]'>
            <svg
              stroke='currentColor'
              fill='currentColor'
              strokeWidth='0'
          ...
          <ul className='group relative'>
            <h5 className='cursor-pointer'>{decodedToken?.username}</h5>
            <ul className='absolute  opacity-0 invisible duration-300 group-hover:opacity-100 group-hover:visible bg-white text-black pl-2 w-[100px] py-4  rounded-sm'>
              <li className='cursor-pointer'>Go To Profile</li>
